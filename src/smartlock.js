...
// 對策：
//  - start 拉到「今天凌晨 00:00」再往前留 8 小時緩衝 → 容忍門鎖「時間慢了」。
//  - end 用台北「隔天 00:00」再往後留 8 小時緩衝 → 容忍門鎖「時間快了」，
//    同時避開 TTLock 會把 endDate 無條件捨去（23:59:59→23:30 提早失效）的問題。
// 註：這是對輕微漂移的容錯；若門鎖時間差很多（整天都開不了），仍需用 TTLock App
//    靠近門鎖以藍牙連線校時（或裝設閘道器自動校時）。
const TTLOCK_DRIFT_MS = 8 * 3600 * 1000  // 容忍門鎖時鐘漂移 ±8 小時
function taipeiTodayWindow() {
  ...
  // start：今天凌晨與「現在往前 8 小時」取較早者，確保剛發的密碼即使門鎖慢了也已生效
...
// 找出綁定此 UID 的房間；若對到多間（換房）→ 以「最新且生效中的合約」為主，避免發舊房
...
async function handleTenantPasscodePrompt(landlordId, lineUserId) {
  ...
  const matched = await matchedRoomsForUser(landlord, lineUserId)
  if (matched.length === 0) {
    return { type: 'text', text: `${who} 您好，您尚未綁定門鎖，請聯絡房東為您設定後即可自助取當日密碼。` }
  }
  const roomLabels = matched.map(r => {
...
async function handleTenantPasscode(landlordId, lineUserId) {
  ...
  // 找出綁定此 UID 的房間（換房時以最新生效合約為主，不發舊房）
  const matched = await matchedRoomsForUser(landlord, lineUserId)
  if (matched.length === 0) {
    return { type: 'text', text: `${who} 您好，您尚未綁定門鎖，請聯絡房東為您設定後即可自助取當日密碼。` }
  }
